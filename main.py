...
from gui import (
    setup_user, validate_user, logout, add_credential_window, view_credentials, 
    delete_credential_window, center_window, WINDOW_WIDTH, WINDOW_HEIGHT, 
    BG_COLOR, BUTTON_BG, BUTTON_FG, HEADER_BG, HEADER_FG, TEXT_COLOR, 
    ACCENT_COLOR, PADDING, create_button, add_credits
)

# Update the show_main_interface function to use the correct window size
def show_main_interface(root) -> None:
    ...
    center_window(main_window, WINDOW_WIDTH, WINDOW_HEIGHT+100)
    ...
    add_btn = create_button(button_frame, "Add Credential", add_credential_window)
    add_btn.pack(pady=5)
    
    view_btn = create_button(button_frame, "View Credentials", view_credentials)
    view_btn.pack(pady=5)
    
    delete_btn = create_button(button_frame, "Delete Credential", delete_credential_window)
...
def main():
    ...
    header_label = tk.Label(header_frame, text="PyPassManager", 
                           font=("Arial", 18, "bold"), bg=HEADER_BG, fg=HEADER_FG)
    header_label.pack(pady=20)

    message_label = tk.Label(content_frame, 
    text="Welcome to PyPassManager!",
